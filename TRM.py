...
class CoCorrecting(BasicTrainer, Loss):   # 在类中再传入类的写法是第一次见
    """
    ...
    def __init__(self, args):
        ...
        self.modelA = self._get_model(self.args.backbone)   #  _get_model是类BasicTrainer中的函数，此时是不是已经让类BasicTrainer初始化了，于是就执行了 self.args.y_file 等这些属性（先这么理解把）
        ...
        self.aum_calculator = AUMCalculator(args.dir, compressed=True)
    ...
    def _compute_loss(self, outputA, outputB, target, target_var, index, epoch, i, parallel=False):
        if epoch <= self.args.stage1:
            ...
            onehot = torch.zeros(target.size(0),
                                 self.args.classnum).scatter_(1, target.view(-1, 1).type(torch.int64), self.args.K)
            onehot = onehot.numpy()
            label = target.to(self.args.device)
            if self.args.stage1 - 10 < epoch <= self.args.stage1:
                self.aum_calculator.update((outputA + outputB) / 2, label, list(index))
            ...
            if self.args.loss_type == 'coteaching':
                ...
                pure_ratio_discard_1, pure_ratio_discard_2 = self.loss_coteaching(outputA, outputB, target_var,
                                                                                  target_var,
                                                                                  forget_rate, ind=index,
                                                                                  loss_type='CE',
                                                                                  noise_or_not=self.noise_or_not,
                                                                                  softmax=True)
            elif self.args.loss_type == 'coteaching_plus':
                lossA, lossB, ind_A_update, ind_B_update, ind_A_discard, ind_B_discard, pure_ratio_1, pure_ratio_2, \
                pure_ratio_discard_1, pure_ratio_discard_2 = self.loss_coteaching_plus(outputA, outputB, target_var,
                                                                                       target_var,
                                                                                       forget_rate, epoch * i, index,
                                                                                       loss_type='CE',
                                                                                       noise_or_not=self.noise_or_not,
                                                                                       softmax=False)
            else:
    ...
    def _cluster_data_into_subsets(self):
        filename = os.path.join(args.dir, 'aum_values.csv')
        curr = []
        ...
        prob_mean = np.array(curr)
        prob_mean1 = prob_mean.copy()
        prob_mean1.sort()
        num_samples = len(prob_mean)
        two_score = math.floor(num_samples / 3)
        three_score = 2 * two_score
        thresholds_2_3 = prob_mean1[three_score]
        thresholds_1_3 = prob_mean1[1]
        thresholds_0_3 = prob_mean1[0]
        self.subset_labels = prob_mean
        np.save(join(self.args.dir, 'subset_labels.npy'), self.subset_labels)
        return thresholds_0_3, thresholds_1_3, thresholds_2_3



    def _get_label_update_stage(self,epoch, thresholds_0_3, thresholds_1_3, thresholds_2_3):

        step = (self.args.stage2 - self.args.stage1) / 2
        if self.args.stage1 <= epoch:
            return thresholds_1_3
        elif self.args.stage1 + step < epoch:
            return thresholds_0_3
        # elif self.args.stage1 + 2 * step < epoch:
        #     return thresholds_0_3
        # return thresholds_0_3
    ...
    def training(self):
        ...
        writer = SummaryWriter(comment=f'_Dataset_{args.dataset}_Rate_{args.noise}_LearningRate_{args.lr}_cluster_mode_{args.cluster_mode}_curriculum_{args.curriculum}')
        for epoch in range(self.args.start_epoch, self.args.epochs): # self.args.start_epoch = 0  self.args.epochs = 320
            print('-----------------')
            self._adjust_learning_rate(epoch)
            # load y_tilde
            if os.path.isfile(self.args.y_file):   #  self.args.y_file 这个参数是怎么形成的？（执行了类BasicTrainer中的函数，于是将BasicTrainer类初始化了，就多了这个self属性）
                self.yy = np.load(self.args.y_file) # /home/wuhaixia/July/Co-teaching_plus/TRM/experiment/test-debug/y.npy
            else:
                self.yy = []
            ...
            if self.args.classnum > 5:
                ...
                test_prec1_A, test_prec1_B, test_prec5_A, test_prec5_B = self.test(epoch,writer)
            else:
                train_prec1_A, train_prec1_B = self.train(epoch)
                val_prec1_A, val_prec1_B = self.val(epoch)
                test_prec1_A, test_prec1_B = self.test(epoch,writer)

            if epoch == self.args.stage1:
                self.aum_calculator.finalize()

            # load best model to modelC    modelC 是指目前最好的网络
            # if epoch < self.args.stage1:   # epoch 到 70 之后 modelC 就不会变了
            #     best_ind = [val_prec1_A, val_prec1_B, self.best_prec1].index(
            #         max(val_prec1_A, val_prec1_B, self.best_prec1))
            #     self.modelC.load_state_dict(
            #         [self.modelA.state_dict(), self.modelB.state_dict(), self.modelC.state_dict()][best_ind])
                # load_state_dict该函数是将预训练的模型参数赋值给当前网络modelC
            is_best = max(val_prec1_A, val_prec1_B) > self.best_prec1  # self.best_prec1是一个准确率，一个数
    ...
    def train(self, epoch=0,writer = None):
        ...
        pure_ratio_discard_B = AverageMeter()    # 这4个变量是什么作用？
        ...
        if epoch >= self.args.stage1:   # self.args.stage1=70 不会随数据集变，一直都是70
            self.new_y = self.yy
        ...
        for i, (input, target, index) in enumerate(self.trainloader):
            ...
            flops, params = profile(self.modelA, inputs=(input,))
            ...
            if epoch < self.args.warmup:   # self.args.warmup = 0     通过 self.args.stage1 控制的参数
                ...
                pure_ratio_discard_1, pure_ratio_discard_2 = self._compute_loss(outputA, outputB, target, target_var,
                                                                                index, epoch, i)
            # loss = (lossA + lossB) / 2
            if epoch > self.args.stage1:  # 70   聚类只进行1次      （是不是可以写成epoch = self.args.stage1 ？）
                thresholds_0_3, thresholds_1_3, thresholds_2_3 = self._cluster_data_into_subsets()
            ...
            if epoch > self.args.stage1 and epoch <= self.args.stage2:  #  self.args.stage1=70，self.args.stage2 =  200
                # using select data sample update parameters, other update label only
                # epoch 在 70 到 200 之间进行标签修正
                yy_A = self.yy
                yy_B = self.yy
                yy_A = torch.tensor(yy_A[index, :], dtype=torch.float32, requires_grad=True, device=self.args.device)
                yy_B = torch.tensor(yy_B[index, :], dtype=torch.float32, requires_grad=True, device=self.args.device)
                ...
                if self.args.mix_grad == 1:   # self.args.mix_grad =1
                ...
                if self.args.discard == 1:    # self.args.discard = 0
                    ...
                    if self.args.curriculum == 0:
                        ...
                        ind_discard = np.unique(np.hstack((ind_A_discard, ind_B_discard)))
                        update_stage = self._get_label_update_stage(epoch,thresholds_0_3, thresholds_1_3, thresholds_2_3)
                        # print(thresholds_0_3) # -3.20194149017334  -15.5789
                        # print(thresholds_1_3) # -0.12620878219604492  -0.24067
                        # print(thresholds_2_3) # 1.1070768237113953    0.96473
                        subset_labels = self.subset_labels[index]
                        # print(subset_labels)
                        # print(self.subset_labels)
                        # print(subset_labels >= update_stage)
                        update_ind = np .where(subset_labels >= update_stage)
                        # print(update_ind)  # array([2, 4, 7, 9])
        ...
        # check label acc
        label_accu_A, label_n2t_A, label_t2n_A = check_label(self.yy, self.clean_labels, self.noise_or_not,
                                                             onehotA=True)
        label_accu_B, label_n2t_B, label_t2n_B = check_label(self.yy, self.clean_labels, self.noise_or_not,
                                                             onehotA=True)
        label_accu_C, label_n2t_C, label_t2n_C = check_label(self.yy, self.clean_labels, self.noise_or_not,
                                                             onehotA=True)
        ...
        # predicted = output_mix.cpu().numpy()
        #
    ...
    def val(self, epoch=0):
        ...
        with torch.no_grad():
            ...
            if self.args.classnum > 5:
                print("\n * Top1 acc:\tA: {:.3f}\tB: {:.3f}\tTop5 acc:\tA: {:.3f}\tB: {:.3f}".format(top1_A.avg,
                                                                                                     top1_B.avg,
                                                                                                     top5_A.avg,
                                                                                                     top5_B.avg))
            else:
    ...
    def test(self, epoch=0, writer = None):
        self.modelA.eval()
        self.modelB.eval()
        ...
        acc, precision, recall, f1, auc_socre = eval_net(self.modelA, self.valloader, self.args.device,
                                                         self.args.classnum)
        # writer.add_scalar('Loss/train_ce', epoch_loss / (n_train // batch_size), epoch)
        writer.add_scalar('Dice/acc', acc, epoch)
        writer.add_scalar('Dice/precision', precision, epoch)
        writer.add_scalar('Dice/recall', recall, epoch)
        writer.add_scalar('Dice/f1', f1, epoch)
        writer.add_scalar('Dice/auc', auc_socre, epoch)
        return top1_A.avg, top1_B.avg
...
if __name__ == "__main__":
    args = get_args()   # 参数空间，包含了必要的参数
    args.dir = os.path.abspath(args.dir)  # 相对路径转绝对路径
    trainer = CoCorrecting(args=args)   # 把参数空间的参数传入 CoCorrecting 类中，在运行这段代码的时候，会运行类的初始化函数，形成类属性，传入的参数args也变成了类属性。所有的类属性都是这种形式 self.
    trainer.training()
