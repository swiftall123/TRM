...
class BasicTrainer(object):
    ...
    def _save_meta(self):
        # save meta data
        print(vars(self.args))   # vars将 object转换成字典对象  vars([object])---->dictionary
        nowTime = datetime.datetime.now().strftime('%Y-%m-%d-%H-%M-%S')
        with open(join(self.args.dir, "settings-{}.json".format(nowTime)), 'w') as f:
            json.dump(vars(self.args), f, indent=4, sort_keys=True)    # 使用json.dump()存储一组数字，编码JSON数据

            # indent=4 这个参数增加了json文件的可读性，双引号使每个键值对都出现在换行符中
    ...
    def _get_model(self, backbone):
        if backbone == 'resnet18':
            ...
            # model.fc = nn.Linear(2048, self.args.classnum)
            ...
            model = CNN(n_outputs=self.args.classnum, input_channel=self.args.input_dim,
                        linear_num=self.args.linear_num).to(self.args.device)
        else:
    ...
    def _get_optim(self, parm, optim="SGD", scheduler=None, lr=None):
        if optim == "SGD" or optim == "sgd":
            optimizer = torch.optim.SGD(parm, lr=lr if lr else self.args.lr, momentum=self.args.momentum,
            ...
            optimizer = torch.optim.RMSprop(parm, lr=lr if lr else self.args.lr, momentum=self.args.momentum,
                                            weight_decay=self.args.weight_decay)
        elif optim == "Adadelta":
    ...
    def _get_dataset_chaoyang(self):
        transform = torchvision.transforms.Compose([
            torchvision.transforms.RandomHorizontalFlip(p=0.5),
            torchvision.transforms.RandomVerticalFlip(p=0.5),
            torchvision.transforms.RandomRotation(degrees=[-180, 180]),
        ...
        trainset = ChaoYang(root=self.args.root,
                            train=0,
                            transform=transform,
                            noise_type=self.args.noise_type,
                            noise_rate=self.args.noise,
                            ...
                            redux=self.args.train_redux,
                            image_size=self.args.image_size)
        testset = ChaoYang(root=self.args.root,
                           train=1,
                           transform=transform1,
                           noise_type='clean',
                           noise_rate=self.args.noise,
                           ...
                           redux=self.args.test_redux,
                           image_size=self.args.image_size)
        valset = ChaoYang(root=self.args.root,
                          train=2,
                          transform=transform1,
                          noise_type='clean',
                          noise_rate=self.args.noise,
    ...
    def _load_data(self):
        ...
        elif self.args.dataset == 'chaoyang':
            trainset, testset, valset = self._get_dataset_chaoyang()
        else:
